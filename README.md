...
## Three First-Class App Types

Agentic OS defines three first-class application citizens, all accessible from the unified **Apps** hub:

| Type | English | Chinese | Definition |
|------|---------|---------|-----------|
| ① | **Agent App** | 智能体应用 | One or more Agents composed into an autonomous application. The primary interaction mode is conversation and task flow — the user states intent, and the Agent executes independently. |
| ② | **Live App** | 灵动应用 | A user-centric interactive application generated by Agent on demand via API and UI composition. Each Live App is persistent — it has its own identity and state, evolves with the user's intent, and can be reused over time. It is not a throw-away widget; it is a living application that belongs to the user. |
| ③ | **Bridge App** | 桥接应用 | A traditional GUI application augmented with an operation Agent layer. The native interface is preserved while the Agent acts as a co-pilot, connecting legacy software to the Agentic OS ecosystem. |
